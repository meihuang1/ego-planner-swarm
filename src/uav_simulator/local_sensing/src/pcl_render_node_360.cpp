...
{
  // 创建深度匹配图像，保持原始深度信息并添加特征点
  cv::Mat depth_matching = depth_mat.clone();
  
  // 将深度图转换为8位图像用于特征检测
  cv::Mat depth_8bit;
  depth_mat.convertTo(depth_8bit, CV_8UC1, 255.0 / 13.0);
  
  // 将单通道深度图转换为3通道BGR图像，以便显示颜色
  cv::Mat depth_bgr;
  cv::cvtColor(depth_8bit, depth_bgr, cv::COLOR_GRAY2BGR);
  ...
  // 随机生成特征点，让分布更自然
  int attempts = 0;
  int max_attempts = num_points * 3;  // 最多尝试3倍数量
  
  while (corners.size() < num_points && attempts < max_attempts) {
  ...
  // 添加一些随机抖动和噪声来模拟ICP匹配过程
  for (size_t i = 0; i < corners.size(); i++) {
    cv::Point2f corner = corners[i];
    
    // 添加随机抖动，模拟匹配误差，增大抖动距离
    ...
    // 绘制原始特征点（亮绿色圆圈），稍微增大尺寸让点更明显
    cv::circle(depth_bgr, corner, 2, cv::Scalar(0, 255, 0), 2);
    
    // 绘制抖动后的匹配点（亮红色圆圈），稍微增大尺寸让点更明显
    cv::circle(depth_bgr, jittered_corner, 2, cv::Scalar(0, 0, 255), 2);
    
    // 绘制连接线（亮黄色），增大距离阈值
    float distance = cv::norm(corner - jittered_corner);
    if (distance < 12.0f) {
      cv::line(depth_bgr, corner, jittered_corner, cv::Scalar(0, 255, 255), 1);
    }
  }
  ...
  depth_matching_msg.image = depth_bgr;
...
void renderSensedPoints(const ros::TimerEvent &)
{
  ...
  render_currentpose();
  render_pcl_world();
  createDepthMatchingImage();  // 添加深度匹配图像生成
}
...
int main(int argc, char **argv)
{
  ...
  nh.param("num_points", num_points, 50);
  ...
  pub_depth_matching = nh.advertise<sensor_msgs::Image>("depth_matching", 10);
